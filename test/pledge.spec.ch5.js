...
describe('The static method `$Promise.resolve`', function(){
  ...
  xit('is a function, and not one we have already written', function(){
    ...
    expect( $Promise.resolve ).not.to.equal( defer().resolve );
    expect( $Promise.resolve ).not.to.equal( Deferral.prototype.resolve );
  });
  ...
  xit('takes a <plain value A> and returns a <promise for A>', function(){
    [42, 'hi', {}, undefined, /cool/, false].forEach(value => {
      ...
      expect( promise._value ).to.equal( value );
    });
  });
  ...
  xit('takes a <promise for A> and returns the same <promise for A>', function(){
    ...
    expect( secondPromise ).to.equal( firstPromise );
  });
  ...
  xit('demonstrates why "resolved" and "fulfilled" are not synonyms', function(){
    ...
    expect( result._state ).to.equal( 'rejected' ); // ...but REJECTED!
...
describe('The static method `$Promise.all`', function(){
  ...
  xit('is a function', function(){
    expect( typeof $Promise.all ).to.equal( 'function' );
  });
  ...
  xit('takes a single array argument', function(){
    ...
    expect( callingAllWithArrays ).not.to.throw();
    ...
    nonArrayValues.forEach(value => {
      ...
      expect( callingAllWithValue ).to.throw( TypeError );
    });
  });
  ...
  xit('converts an <array of values> into a <promise for an array of values>', function(){
    ...
    return expect( promise ).to.eventually.deep.equal( values );
  });
  ...
  xit('converts an <array of values and promises> into a <promise for an array of values>', function(){
  ...
  xit('converts an <array of async promises> into a <promise for an array of values>', function(){
  ...
  xit('converts an <array of async promises> (fulfilling in random order) into a <promise for an array of values> (ordered by index in the original array)', function(){
  ...
  xit('rejects with <reason E> when one of the input promises rejects with <reason E>', function(){
    ...
    return expect( promise ).to.be.rejectedWith( 'any Black Mirror episode');
  });

  // This probably already passes, but let's be sure. We're strict that way.
  xit('is not affected by additional rejections', function(){
    ...
    return expect( promise ).to.be.rejectedWith( 'I am the first rejection');
  });
